---
title: "Data Exploration 3G - Navigation Performance vs Energy Efficiency"
...
```{r, fig.width=4,fig.height=3,collapse=TRUE}
boxplot(threeG_nav_df$batteryStatsAvg,names=c("Nav Mean(3G)"))
```
...
```{r}
bartlett.test(efficiency~performance, data = threeG_nav_df)
```


**multiple comparison**
From above, we can see there is some difference between groups but we don't know which group is higher or lower than other groups. To find it out, we will do the multiple comparison.
```{r, fig.width=9, fig.height=3, collapse=TRUE}
TukeyHSD(threeG_nav_df.aov)
par(las=1)
plot(TukeyHSD(threeG_nav_df.aov))
```
From the result of the mutiple comparison, we can see there is some statistical significance between the group **low** and **average**, also **low** and **high**.
