...
2. __timeToFirstByte__ - The amount of time ittakes for the server to send the first payload to the client
3. __downloadTime__ - Response time only (download)
4. __totalTime__ - Request plus response time (network only)
5. __totalLoadTime__ - 
6. __fcp__ - The time from when thepage starts loading to when the first bit of meaningful content is rendered
7. __fp__ - The exact time taken for the browser to render anything as visually different from what was on the screen before navigation
Blocking factor: 
...
```{r}
write.csv(wifi_navigation_df,"NavigationTiming.csv", row.names = TRUE)
```

Save transformed data for load speed:
```{r}
write.csv(wifi_load_df,"LoadSpeed.csv", row.names = TRUE)
```
